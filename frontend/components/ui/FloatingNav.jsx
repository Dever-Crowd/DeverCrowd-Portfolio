...
import { RiHomeLine, RiQuestionMark, RiStackLine, RiStarLine } from "react-icons/ri";
...
const FloatingNav = () => {
    ...
    return (
        <>
            {/* Desktop — شمال عمودي */}
            <div
                className="hidden md:flex fixed left-6 top-1/4 -translate-y-1/2 z-50 flex-col gap-1.5 p-2.5 rounded-2xl border border-white/10 backdrop-blur-md"
                    ...
                    >
                        <Icon
                            style={{
                                width: 17,
                                height: 17,
                                color: active === id ? "var(--primary)" : "rgba(255,255,255,0.35)",
            ...
            {/* Mobile — تحت أفقي */}
            <div
                className="flex md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex-row gap-1.5 p-2.5 rounded-2xl border border-white/10 backdrop-blur-md"
                style={{ background: "color-mix(in srgb, var(--card) 80%, transparent)" }}
            >
                {sections.map(({ id, label, icon: Icon }) => (
                    <button
